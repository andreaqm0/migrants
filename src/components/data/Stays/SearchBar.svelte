<script lang="ts">
  ...
  import { getStaysFilteringParams } from "@/services/utils";
  ...
  function clear() {
    const url = new URL("/stay-reports", window.location.origin);
    window.location.assign(url.toString());
  }

  onMount(() => {
    const params = getStaysFilteringParams();
    searchString = params.searchString;
    nationality = params.nationality;
    sortBy = params.sortBy;
    sortOrder = params.sortOrder;
    from = new Date(params.from).toISOString().split('T')[0]
    to = params ? new Date(params.to).toISOString().split('T')[0] : undefined
  });
</script>
        ...
        <div class="flex flex-col gap-3" id="form-filter">
          ...
          <div>
            ...
            <div class="form-control w-full">
                ...
                <input class="input input-bordered" type="date" name="from" bind:value={from} />
            </div>
            <div class="form-control w-full">
                <label for="" class="label">
                    <span class="label-text">Hasta:</span>
                </label>
                <input class="input input-bordered" type="date" name="to" bind:value={to} />
            </div>
          </div>
          <div>
            ...
            <div class="form-control">
              <label class="label cursor-pointer">
                ...
                <input
                  ...
                  value="arrivalDate"
                  class="radio radio-sm"
                  checked={sortBy === "arrivalDate" || !sortBy}
                />
              </label>
            </div>
            <div class="form-control">
